...
function Navigation(props) {
  ...
  const name = userData ? userData.name : "";// отображение почты в шапке
    ...
    (loggedIn) ?
      <nav className="header__nav">
        ...
        <div className={NavClassName}>
          ...
          <NavLink to="/profile" className="header__nav-profile" activeClassName="active-link" onClick={handleMenuClick}>{name}<img className='nav__user-icon' src={user} alt='Фото пользователя' /></NavLink>
        </div>
      </nav>
      :
      <nav className="header__nav">
